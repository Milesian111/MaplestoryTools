"""
运行此脚本可将 dragon.py 打包为 exe（Dragon GUI）。

打包参数：
- 单文件：--onefile
- 无控制台：--windowed
- 管理员启动：--uac-admin
- 资源目录：picture（可选 icon）
- 运行时配置：与 exe 同目录生成/读取 dragon_settings.ini
"""
...
def main() -> None:
    version = get_version()
    name = f"好厉害屠龙v{version}"
    entry = PROJECT_DIR / "dragon.py"
    icon_ico = PROJECT_DIR / "icon" / "icon.ico"
