...
if __name__ == "__main__":
    print("开始查找图片...")
    print(f"需要找到 {REQUIRED_MATCH_COUNT} 个不同位置的 cd1.png 才会停止\n")
    ...
    while True:
        ...
        print(f"\n找到 {found_count} 个不同位置的 cd1.png")
        
        # 如果找到2个不同位置，beep并停止
        if found_count >= REQUIRED_MATCH_COUNT:
            print(f"✓ 找到 {found_count} 个不同位置的 cd1.png，达到阈值 {REQUIRED_MATCH_COUNT}，停止任务")
            winsound.Beep(1000, 200)  # 频率1000Hz，持续时间200ms
            break
        else:
            print(f"✗ 只找到 {found_count} 个不同位置，未达到阈值 {REQUIRED_MATCH_COUNT}，继续循环")
            # 执行点击序列
